...
if( $f_error || $f_cookie_error ) {
	echo '<div class="msg-error ui-state-highlight ui-corner-all">';
	echo '<ul>';
...
<div id="login-div" class="form-container">
		<fieldset>
			<div class="field-container">
				...
				<span class="input"><input id="username" type="text" name="username" size="40" maxlength="<?php echo USERLEN;?>" value="<?php echo string_attribute( $f_username ); ?>" class="<?php echo $t_username_field_autofocus ?>" /></span>
				<span class="label-style"></span>
			</div>
			<div class="field-container">
				<label for="password"><span><?php echo lang_get( 'password' ) ?></span></label>
				<span class="input"><input id="password" type="password" name="password" size="40" maxlength="<?php echo PASSLEN;?>" class="<?php echo $t_password_field_autofocus ?>" /></span>
				<span class="label-style"></span>
			</div>
			<div class="field-container">
				<label for="remember-login"><span><?php echo lang_get( 'save_login' ) ?></span></label>
				<span class="input check-box"><input id="remember-login" type="checkbox" name="perm_login" <?php echo ( $f_perm_login ? 'checked="checked" ' : '' ) ?>/>Yes please</span>
				<span class="label-style"></span>
			</div>
...
if ( config_get_global( 'admin_checks' ) == ON ) {
	...
	if( count( $t_warnings ) > 0 ) {
		echo '<div class="msg-error ui-state-error ui-corner-all">';
		echo '<ul>';
